// HUSH — a private, fractional-algorithmic stablecoin on Robinhood Chain (Phase I: off-chain ledger, real privacy primitives).
//   hUSD  : the stablecoin, pegged to $1 (fractional-algorithmic, Frax-style)
//   HUSH  : the governance/share token ($HUSH on Robinhood Chain)
//   Privacy: shield hUSD -> hold a private balance -> send shielded (amount + parties hidden).
...
// ---------- protocol state ----------
let db = {
  husdPrice: 1.0, husdSupply: 1_250_000, hushPrice: 0.85, hushSupply: 100_000_000,
  cr: 0.9,                       // collateral ratio (fractional-algorithmic)
  collateralUsd: 1_125_000,      // ETH+USDG reserves backing hUSD
...
// ---------- THE SHRED: punch service charge -> HUSH buyback & burn ----------
...
function shredBurn(now) {
  ...
  const usd = shred.svcUsd; const px = Math.max(0.0001, db.hushPrice); const hush = usd / px;
  db.hushSupply = Math.max(0, db.hushSupply - hush); shred.svcUsd = 0; shred.burnedHush += hush; shred.burnedUsd += usd; shred.epochs++;
  const id = base58(sha('shred|' + shred.epochs + '|' + usd.toFixed(6) + '|' + hush.toFixed(6) + '|' + now));
  shred.burns.unshift({ id, usd, hush, px, ts: now, epoch: shred.epochs }); if (shred.burns.length > 40) shred.burns.pop();
}
...
function W(a) { a = a.toLowerCase(); return db.wallets[a] || (db.wallets[a] = { usdg: SEED.usdg, hush: SEED.hush, husd: SEED.husd, priv: SEED.priv, seeded: true }); }
...
const CHAIN = { ok: false, block: 0, treasuryUsdg: 0, treasuryHush: 0, lastRead: 0, errs: 0 };
...
// ---------- THE HAPPY: temporary hUSD staking, paid in $HUSH from a FIXED pre-funded pool. No printing. Hard end. ----------
const HAPPY = {
  apy: +(process.env.HAPPY_APY || 0.40),               // 40% APY, in hUSD terms
  pool: +(process.env.HAPPY_POOL || 5_000_000),         // HUSH set aside by the treasury for the whole season
  cap: +(process.env.HAPPY_CAP || 250_000),             // max hUSD staked protocol-wide
  start: +(process.env.HAPPY_START || 1),                // open now
  end: +(process.env.HAPPY_END || 1792886400000),       // 2026-10-25 00:00 UTC, then it is over
};
if (!db.happy) db.happy = { staked: 0, paidHush: 0, paidUsd: 0, stakers: 0 };
...
function happyLive(now) { return now >= HAPPY.start && now < HAPPY.end && db.happy.paidHush < HAPPY.pool; }
function accrue(u, now) {   // reward accrues in hUSD terms per second while happy hour is live
...
function happyView(u, now) { accrue(u, now); const px = Math.max(0.000001, db.hushPrice); return { staked: u.stake || 0, accruedUsd: u.stakeAcc || 0, accruedHush: (u.stakeAcc || 0) / px, since: u.stakeSince || null }; }

// ---------- PHASE III: The Note (pay links) + The Carbon Copy (view keys) ----------
//   The Note : lock shielded hUSD behind a secret; anyone holding the link claims it into their own shielded balance.
...
// ---------- BONDS: USDG in, discounted $HUSH out, vested. The USDG stays in reserve and mints NOTHING, so every bond over-collateralizes hUSD. ----------
const BOND = {
  ...
  // THE FREEZER: lock the bond. Deeper discount, hard 48-hour lock, and the locked HUSH earns APY in HUSH paid from the Happy Hour pool (fixed, no printing).
...
if (!db.bonds) db.bonds = { soldUsd: 0, soldHush: 0, n: 0, day: 0, dayUsd: 0 };
if (!db.freezer) db.freezer = { lockedHush: 0, usd: 0, n: 0, yieldHush: 0 };
const freezerPrice = () => Math.max(0.000001, db.hushPrice) * (1 - BOND.lockDiscount);
const freezerYield = (b, now) => b.lock ? b.hush * BOND.lockApy * (Math.min(now, b.ts + BOND.lockMs) - b.ts) / 31536000000 : 0;
function bondDay() { const d = Math.floor(Date.now() / 864e5); if (db.bonds.day !== d) { db.bonds.day = d; db.bonds.dayUsd = 0; } return db.bonds; }
const bondPrice = () => Math.max(0.000001, db.hushPrice) * (1 - BOND.discount);
function bondView(u, now) { const list = (u.bonds || []).map((b) => { const len = b.lock ? BOND.lockMs : BOND.vestMs; let k = Math.min(1, Math.max(0, (now - b.ts) / len)); if (b.lock && k < 1) k = 0; const vested = b.hush * k; const y = freezerYield(b, now); return { id: b.id, usd: b.usd, hush: b.hush, price: b.price, ts: b.ts, lock: !!b.lock, vestEnd: b.ts + len, vested, yieldHush: y, claimable: Math.max(0, vested - b.claimed) + (b.lock && k >= 1 && !b.yieldPaid ? y : 0), claimed: b.claimed }; }); return { list, claimable: list.reduce((x, b) => x + b.claimable, 0), pending: list.reduce((x, b) => x + (b.hush - b.claimed), 0), locked: list.filter((b) => b.lock && b.claimed < b.hush).reduce((x, b) => x + b.hush, 0), freezing: list.filter((b) => b.lock && b.claimed < b.hush).reduce((x, b) => x + b.yieldHush, 0) }; }

// ---------- THE DARK POOL: shielded 1x exposure to tokenized stocks, priced off the exchange tape, settled in shielded hUSD ----------
...
// ---------- $HUSH price: Robinhood Chain pools (DexScreener) when HUSH_MINT is set ----------
let HUSH_LIVE = { px: 0, liq: 0, pair: '', t: 0 };
async function pollHush() {
  if (!HUSH_MINT) return;
  try { const r = await fetch('https://api.dexscreener.com/latest/dex/tokens/' + HUSH_MINT); if (!r.ok) return;
    ...
    if (ps[0]) { HUSH_LIVE = { px: +ps[0].priceUsd, liq: (ps[0].liquidity && ps[0].liquidity.usd) || 0, pair: ps[0].pairAddress || '', t: Date.now() }; db.hushPrice = HUSH_LIVE.px; } } catch (e) {}
}
setInterval(pollHush, 20000); pollHush();

// ---------- peg / algo tick ----------
function tick() {
  const now = Date.now(); const dt = (now - db.lastTick) / 1000; if (dt < TICK_SEC) return; db.lastTick = now;
  // mean-reverting hUSD price around $1
  db.husdPrice += (1 - db.husdPrice) * 0.18 + (Math.random() - 0.5) * 0.0035;
  db.husdPrice = Math.max(0.97, Math.min(1.03, db.husdPrice));
  // algorithmic collateral ratio: above peg -> lower CR (more algo); below -> raise CR (more backing)
  const target = db.husdPrice > 1.001 ? db.cr - 0.01 : db.husdPrice < 0.999 ? db.cr + 0.01 : db.cr;
  db.cr += (Math.max(0.55, Math.min(1, target)) - db.cr) * 0.25;
  // HUSH price drifts (captures protocol value)
  if (!HUSH_LIVE.px) db.hushPrice = Math.max(0.05, db.hushPrice * (1 + (Math.random() - 0.49) * 0.02));
...
function metrics() {
  const backing = db.collateralUsd / Math.max(1, db.husdSupply);
  return {
    stable: STABLE, gov: GOV, mint: HUSH_MINT, treasury: TREASURY, network: 'robinhood', chainId: 4663, explorer: 'https://explorer.mainnet.chain.robinhood.com', hushLive: HUSH_LIVE.px ? HUSH_LIVE : null, peg: 1.0,
    husdPrice: +db.husdPrice.toFixed(4), pegStatus: db.husdPrice >= 1.001 ? 'above' : db.husdPrice <= 0.999 ? 'below' : 'at',
    husdSupply: db.husdSupply, husdMarketCap: db.husdPrice * db.husdSupply,
    cr: db.cr, collateralUsd: db.collateralUsd, backingRatio: backing,
    hushPrice: db.hushPrice, hushSupply: db.hushSupply, hushMarketCap: db.hushPrice * db.hushSupply,
    minDeposit: MIN_DEPOSIT, chain: { ok: CHAIN.ok, block: CHAIN.block, treasuryUsdg: CHAIN.treasuryUsdg, treasuryHush: CHAIN.treasuryHush, lastRead: CHAIN.lastRead, usdg: USDG.addr, rpc: RPCS[0] },
...
http.createServer(async (req, res) => {
  ...
  if (u === '/api/config') return json(res, 200, { stable: STABLE, gov: GOV, mint: HUSH_MINT, treasury: TREASURY, network: 'robinhood', chainId: 4663, explorer: 'https://explorer.mainnet.chain.robinhood.com', hushLive: HUSH_LIVE.px ? HUSH_LIVE : null });
  ...
  if (req.method === 'POST') {
    ...
    if (u === '/api/mint') { // collateral + HUSH -> hUSD
      const m = num(d.amount); if (!m) return json(res, 200, { error: 'enter an amount' });
      const needUsdg = m * db.cr; const algoUsd = m * (1 - db.cr); const burnHush = algoUsd / db.hushPrice;
      if (w.usdg < m) return json(res, 200, { error: 'not enough USDG — deposit first' });
      // the algorithmic slice is paid in USDG too: it buys HUSH at market and burns it via the shred (nothing is printed to mint hUSD)
      w.usdg -= m; w.husd += m; db.husdSupply += m; db.collateralUsd += needUsdg; db.hushSupply = Math.max(0, db.hushSupply - burnHush);
      shred.svcUsd += algoUsd; save();
      return json(res, 200, { ok: true, minted: m, usedUsdg: needUsdg, algoUsd, burnedHush: burnHush, ...account(d.wallet) });
    }
    ...
    if (u === '/api/bond') { // USDG ledger -> discounted HUSH, vested. USDG stays in reserve. Nothing minted.
      ...
      const lock = !!d.lock; const price = lock ? freezerPrice() : bondPrice(); const hush = x / price;
      w.usdg -= x; db.collateralUsd += x; B.dayUsd += x; B.soldUsd += x; B.soldHush += hush; B.n++; if (lock) { db.freezer.lockedHush += hush; db.freezer.usd += x; db.freezer.n++; }
      w.bonds = w.bonds || []; w.bonds.push({ id: base58(randomBytes(6)), usd: x, hush, price, ts: now, claimed: 0, lock }); hist(w, { type: lock ? 'freezer' : 'bond', amt: x }); save();
      return json(res, 200, { ok: true, bonded: x, hushOut: hush, price, market: db.hushPrice, lock, unlockAt: lock ? now + BOND.lockMs : now + BOND.vestMs, apy: lock ? BOND.lockApy : 0, ...account(d.wallet) });
    }
    if (u === '/api/bond/claim') { const now = Date.now(); let got = 0, frozen = 0; for (const b of w.bonds || []) { if (b.lock) { if (now < b.ts + BOND.lockMs) continue; const c = b.hush - b.claimed; if (c > 0) { b.claimed = b.hush; got += c; db.freezer.lockedHush = Math.max(0, db.freezer.lockedHush - c); } if (!b.yieldPaid) { let y = freezerYield(b, now); const left = HAPPY.pool - db.happy.paidHush; if (y > left) y = Math.max(0, left); b.yieldPaid = true; frozen += y; db.happy.paidHush += y; db.happy.paidUsd += y * db.hushPrice; db.freezer.yieldHush += y; } continue; } const k = Math.min(1, (now - b.ts) / BOND.vestMs); const c = Math.max(0, b.hush * k - b.claimed); b.claimed += c; got += c; } if (got + frozen < 1e-9) return json(res, 200, { error: 'nothing vested or unlocked yet' }); w.hush += got + frozen; save(); return json(res, 200, { ok: true, claimedHush: got + frozen, frozenHush: frozen, ...account(d.wallet) }); }
    if (u === '/api/withdraw') { // ledger -> payout queue (treasury pays by hand, then marks it paid). asset: USDG (default) or HUSH
      const asset = d.asset === 'HUSH' ? 'HUSH' : 'USDG';
      const x = num(d.amount, asset === 'HUSH' ? w.hush : w.usdg); if (!x) return json(res, 200, { error: 'nothing to withdraw' }); if (asset === 'USDG' && x < 1) return json(res, 200, { error: 'minimum 1 USDG' });
      if (asset === 'HUSH') w.hush -= x; else w.usdg -= x;
    ...
    if (u === '/api/stake') { // hUSD -> happy hour
      const now = Date.now(); if (!happyLive(now)) return json(res, 200, { error: 'happy hour is not open' });
      const x = num(d.amount, w.husd); if (!x) return json(res, 200, { error: 'nothing to stake' });
      if (db.happy.staked + x > HAPPY.cap) return json(res, 200, { error: 'happy hour is full — cap ' + HAPPY.cap.toLocaleString() + ' hUSD' });
      accrue(w, now); if (!w.stake) { db.happy.stakers++; w.stakeSince = now; } w.husd -= x; w.stake = (w.stake || 0) + x; w.stakeT = now; db.happy.staked += x; save();
    ...
    if (u === '/api/unstake') { const now = Date.now(); accrue(w, now); const x = num(d.amount, w.stake || 0); if (!x) return json(res, 200, { error: 'nothing staked' });
      const xn = svc('unshield', x, w); w.stake -= x; w.husd += xn; db.happy.staked = Math.max(0, db.happy.staked - x); if (w.stake <= 0) { w.stake = 0; db.happy.stakers = Math.max(0, db.happy.stakers - 1); } save();
    ...
    if (u === '/api/claim') { const now = Date.now(); accrue(w, now); const usd = w.stakeAcc || 0; if (usd < 0.01) return json(res, 200, { error: 'nothing to claim yet' });
      const px = Math.max(0.000001, db.hushPrice); let hush = usd / px; const left = HAPPY.pool - db.happy.paidHush; if (left <= 0) return json(res, 200, { error: 'the pool is spent — happy hour is over' }); if (hush > left) hush = left;
      w.stakeAcc = 0; w.hush += hush; db.happy.paidHush += hush; db.happy.paidUsd += hush * px; save();
      return json(res, 200, { ok: true, claimedHush: hush, claimedUsd: hush * px, ...account(d.wallet) });
    }
    if (u === '/api/redeem') { // hUSD -> collateral + HUSH
      const r = num(d.amount, w.husd); if (!r) return json(res, 200, { error: 'nothing to redeem' });
      const rn = svc('redeem', r, w); const outUsdg = rn * db.cr; const mintHush = (rn * (1 - db.cr)) / db.hushPrice;
      w.husd -= r; w.usdg += outUsdg; w.hush += mintHush; db.husdSupply = Math.max(0, db.husdSupply - r); db.collateralUsd = Math.max(0, db.collateralUsd - outUsdg); db.hushSupply += mintHush; save();
      return json(res, 200, { ok: true, redeemed: r, gotUsdg: outUsdg, gotHush: mintHush, ...account(d.wallet) });
    }
    if (u === '/api/note/create') { // lock shielded hUSD behind a secret link
      const x = num(d.amount, w.priv); if (!x) return json(res, 200, { error: 'not enough private balance' }); if (x < 1) return json(res, 200, { error: 'minimum 1 hUSD' });
    ...
    if (u === '/api/dark/open') { // shielded hUSD -> a hidden position
    ...
    if (u === '/api/shield') { // public hUSD -> private
      const s = num(d.amount, w.husd); if (!s) return json(res, 200, { error: 'nothing to shield' });
      const sn = svc('shield', s, w); w.husd -= s; w.priv += sn; sh.totalValue += sn; hist(w, { type: 'shield', amt: sn }); const { C, note } = shieldNote(sn, shKeys[0].pub);
    ...
    if (u === '/api/unshield') { // private -> public
      ...
      const unn = svc('unshield', un, w); w.priv -= un; w.husd += unn; hist(w, { type: 'unshield', amt: un }); sh.totalValue = Math.max(0, sh.totalValue - un); sh.nullifiers++;
  ...
  serve(req, res);
}).listen(PORT, () => console.log('HUSH (' + STABLE + '/' + GOV + ') on Robinhood Chain · :' + PORT));
