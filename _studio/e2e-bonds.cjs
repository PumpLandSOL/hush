// Bonds + happy boost + HUSH payout queue E2E (dev server, DEV_FAUCET=1).
const B = 'http://localhost:8212'; const U = '0x0000000000000000000000000000000000000309';
...
(async () => {
  ...
  ok('bond 200 USDG → HUSH at discount', b.ok && f(b.bonded) === 200 && Math.abs(b.hushOut - 200 / (b.market * 0.8)) < 1e-6, `${Math.round(b.hushOut)} HUSH @ ${b.price}`);
  ok('USDG left ledger, nothing minted', f(b.usdg) === 800 && f(b.husd) === 0);
  ...
  ok('reserve grew by 200 (over-collateralizes hUSD)', f(m1.collateralUsd - col0) === 200, 'Δ ' + f(m1.collateralUsd - col0));
  ...
  const cl = await post('/api/bond/claim', {}); ok('claim pays the vested sliver', cl.ok && cl.claimedHush > 0 && cl.hush > 0, 'claimed ' + cl.claimedHush.toFixed(4));
  const wd = await post('/api/withdraw', { asset: 'HUSH', amount: cl.hush }); ok('HUSH withdrawal queued', wd.ok && wd.queued.asset === 'HUSH' && f(wd.hush) === 0, 'id ' + wd.queued.id);
