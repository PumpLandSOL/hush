...
const B = 'http://localhost:' + (process.env.PORT || 8212); const A = '0x00000000000000000000000000000000000000f1', C = '0x00000000000000000000000000000000000000f2', D = '0x00000000000000000000000000000000000000f3';
...
(async () => {
  ...
  // earned hUSD is real: A uncloaks everything and redeems
  const u = await post('/api/unshield', A, { amount: a3.priv }); ok('earned hUSD uncloaks like any other', u.ok && u.husd > 2000 + 2980, 'husd ' + u.husd.toFixed(2));
