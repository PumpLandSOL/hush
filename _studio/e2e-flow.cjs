...
const B = 'http://localhost:8212'; const W = '0x00000000000000000000000000000000000000b2';
...
(async () => {
  ...
  ok('mint 500 hUSD costs 500 USDG total', mt.ok && f(mt.usdg) === 500 && f(mt.husd) === 500, `usdg=${f(mt.usdg)} husd=${f(mt.husd)} collat=${f(mt.usedUsdg)} algo=${f(mt.algoUsd)}`);
  ...
  const st = await post('/api/stake', { amount: 200 }); ok('stake 200', st.ok && f(st.husd) === 300 && f(st.happy.staked) === 200, JSON.stringify(st.happy));
  await new Promise((r) => setTimeout(r, 3000));
  const ac = await post('/api/account', {}); ok('reward accrues in hUSD terms', ac.happy.accruedUsd > 0 && ac.happy.accruedHush > 0, `accruedUsd=${ac.happy.accruedUsd.toExponential(2)} accruedHush=${f(ac.happy.accruedHush)}`);
  const cl = await post('/api/claim', {}); ok('claim gated below $0.01 or pays HUSH', cl.error === 'nothing to claim yet' || (cl.ok && cl.claimedHush > 0), cl.error || 'claimed ' + f(cl.claimedHush));
  const us = await post('/api/unstake', { amount: 200 }); ok('unstake 200 minus 30bps service charge', us.ok && f(us.unstaked) === 199.4 && f(us.fee) === 0.6, `back ${f(us.unstaked)} husd=${f(us.husd)}`);
  const rd = await post('/api/redeem', { amount: 100 }); ok('redeem 100 hUSD pays CR in USDG (net of 50bps)', rd.ok && rd.gotUsdg > 0, `usdg +${f(rd.gotUsdg)} hush +${f(rd.gotHush)} now usdg=${f(rd.usdg)}`);
