...
const B = 'http://localhost:8212'; const U = '0x0000000000000000000000000000000000000311';
...
(async () => {
  ...
  ok('freezer 200 USDG → HUSH at −30%', b.ok && b.lock && Math.abs(b.hushOut - 200 / (b.market * 0.7)) < 1e-6, `${Math.round(b.hushOut)} HUSH @ ${b.price} apy ${b.apy}`);
  ...
  const m1 = await (await fetch(B + '/api/metrics')).json(); ok('freezer stats + reserve grew', m1.bonds.freezer.n >= 1 && m1.bonds.freezer.lockedHush > 0 && f(m1.collateralUsd - m0.collateralUsd) === 200);
  await new Promise((r) => setTimeout(r, 3500));
  const paid0 = m1.happy.paidHush;
  const c = await post('/api/bond/claim', {}); const exp = b.hushOut * 0.8 * (+process.env.LOCK_MS || 3000) / 31536000000;
  ok('unlock pays principal + frozen yield', c.ok && c.frozenHush > 0 && Math.abs(c.claimedHush - (b.hushOut + c.frozenHush)) < 1e-6, `principal ${Math.round(b.hushOut)} yield ${c.frozenHush.toExponential(3)} (expected ~${exp.toExponential(3)})`);
  const m2 = await (await fetch(B + '/api/metrics')).json();
  ok('yield drawn from the Happy Hour pool, not printed', m2.happy.paidHush > paid0 && Math.abs(m2.happy.paidHush - paid0 - c.frozenHush) < 1e-9);
  ok('freezer lockedHush released', m2.bonds.freezer.lockedHush < m1.bonds.freezer.lockedHush);
