...
let wallet = localStorage.getItem('hush_w') || '';
let refParam = ''; try { const q = new URLSearchParams(location.search); if (/^0x[a-fA-F0-9]{40}$/.test(q.get('ref') || '')) { refParam = q.get('ref').toLowerCase(); localStorage.setItem('hush_ref', refParam); } else refParam = localStorage.getItem('hush_ref') || ''; } catch (e) {}
...
async function connectPhantom() {                                // EVM wallet on Robinhood Chain (name kept for the call sites)
  ...
  try {
    ...
    wallet = pk; localStorage.setItem('hush_w', pk); setConnected(); toast('wallet connected · Robinhood Chain'); await loadAccount();
  } catch (e) { toast('connection cancelled', true); }
}
if (window.ethereum && window.ethereum.on) window.ethereum.on('accountsChanged', (acc) => { if (acc && acc.length) { wallet = acc[0].toLowerCase(); localStorage.setItem('hush_w', wallet); setConnected(); loadAccount(); } });
$('connect').onclick = async () => {
  if (wallet) {                                                   // already connected -> disconnect
    wallet = ''; localStorage.removeItem('hush_w'); A = null; setConnected(); renderAccount(); toast('disconnected'); return;
  }
...
function renderMetrics() {
  if (!M) return;
  const pegEl = $('s-peg'); pegEl.textContent = '$' + fmt(M.husdPrice, 4); pegEl.className = 'v peg ' + M.pegStatus;
  $('s-cr').textContent = fmt(M.cr * 100, 1) + '%';
  $('s-col').textContent = '$' + big(M.collateralUsd);
  $('s-sup').textContent = big(M.husdSupply) + ' hUSD';
  $('s-shd').textContent = big(M.shielded.totalValue) + ' hUSD';
  ...
  if (M.happy) { const V = M.happy;
    ...
    $('v-pool').textContent = big(V.poolLeft) + ' HUSH'; $('v-ends').textContent = V.startsIn > 0 ? 'opens in ' + dur(V.startsIn) : V.live ? dur(V.endsIn) : 'ended'; $('v-n').textContent = fmt(V.stakers, 0);
  }
...
function renderPanel() {
  ...
  if (tab === 'seal') {
    ...
    p.innerHTML = `<div class="note">Mint $1-pegged <b>hUSD</b> with deposited USDG: <b>${fmt(cr * 100, 0)}% goes to collateral</b>, the <b>${fmt((1 - cr) * 100, 0)}% algorithmic share buys HUSH at market and burns it</b>. Nothing is printed.</div>
      <div class="field"><input id="in" type="number" placeholder="0.00" min="0"><span class="u">hUSD</span></div>
      <div class="kv"><span>USDG collateral (${fmt(cr * 100, 0)}%)</span><b id="o1">—</b></div><div class="kv"><span>USDG → buys &amp; burns HUSH (${fmt((1 - cr) * 100, 0)}%)</span><b id="o2">—</b></div>
      <button class="btn wide" id="act" style="margin-top:14px">Mint hUSD</button>`;
    ...
    $('mx').onclick = () => { if (A) $('in').value = A.husd; };
    $('act').onclick = () => doAct('/api/redeem', { amount: +$('in').value }, (r) => `redeemed ${fmt(r.redeemed, 0)} hUSD`);
    $('act2').onclick = () => doAct('/api/unshield', { amount: +$('in').value }, (r) => `unshielded ${fmt(r.unshielded, 2)} hUSD`);
